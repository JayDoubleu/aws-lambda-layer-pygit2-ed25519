...
RUN yum -y install python-virtualenv libffi-devel libcurl-devel cmake autoreconf git curl http-parser-devel libgpg-error-devel zlib-devel perl-core lksctp-tools-devel python-pip python-devel zip
...
RUN mkdir -p /tmp/lambda_layer/python/lib/python2.7/site-packages/
WORKDIR /tmp/lambda_layer
RUN mv /tmp/venv/lib/python2.7/site-packages/* /tmp/lambda_layer/python/lib/python2.7/site-packages/
RUN mv /tmp/static_libs/* /tmp/lambda_layer/python/lib/python2.7/site-packages/
RUN rm -f python/lib/python2.7/site-packages/create_test_py.sh
